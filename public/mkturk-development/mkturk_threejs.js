  ...
  for (
    ...
    for (cam in IMAGES[taskscreen][classlabel].CAMERAS) {
      ...
      var camera = new THREE.PerspectiveCamera(
        IMAGES[taskscreen][classlabel].CAMERAS[cam].fieldOfVIEW,
      ...
      var deltavec = [
        unitscreen.x - originscreen.x,
        unitscreen.y - originscreen.y,
      ];
      IMAGEMETA[taskscreen + "OriginScreenPixels"] = originscreen;
      // IMAGES[taskscreen].originscreenPixels = originscreen
      IMAGEMETA[taskscreen + "THREEJStoPixels"] = Math.max.apply(
        null,
        deltavec
      );
      IMAGEMETA[taskscreen + "THREEJStoInches"] =
        IMAGEMETA[taskscreen + "THREEJStoPixels"] / ENV.ViewportPPI;

      //FOR CAMERA position
      IMAGES[taskscreen][classlabel].CAMERAS[cam].positionInches = {};
      for (keys in IMAGES[taskscreen][classlabel].CAMERAS[cam].position) {
        IMAGES[taskscreen][classlabel].CAMERAS[cam].positionInches[keys] =
          rescaleArrayInchestoTHREEJS(
            IMAGES[taskscreen][classlabel].CAMERAS[cam].position[keys],
            1 / IMAGEMETA[taskscreen + "THREEJStoInches"]
          );
      }
...
function updateObjectSingleFrame(
  ...
  var boundingBox = {
    x: [
      twodcoord_min.x +
        (scenecenterX - IMAGEMETA[taskscreen + "OriginScreenPixels"].x),
      twodcoord_max.x +
        (scenecenterX - IMAGEMETA[taskscreen + "OriginScreenPixels"].x),
    ].sort(function (a, b) {
      return a - b;
    ...
    y: [
      twodcoord_max.y +
        CANVAS.offsettop +
        (scenecenterY - IMAGEMETA[taskscreen + "OriginScreenPixels"].y),
      twodcoord_min.y +
        CANVAS.offsettop +
        (scenecenterY - IMAGEMETA[taskscreen + "OriginScreenPixels"].y),
    ].sort(function (a, b) {
