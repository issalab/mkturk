...
export const dataSlice = createSlice({
  ...
  initialState,
  reducers: {
    setDatafileList: (state, action) => {
      state.list = action.payload;
    },
    setCurrentDatafile: (state, action) => {
      state.current = action.payload;
    },
    setCurrentDatafileGeneration: (state, action) => {
      state.current.metadata = action.payload;
    },
...
export const selectCurrentData = (state: RootState) => {
  if (state.storage.current.data !== undefined) {
